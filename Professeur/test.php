...
if (isset($_SESSION['professeur'])) {
	$sth = $db->prepare("SELECT d.nom_dprtm from departement d inner join filiere f on d.id_dprtm=f.id_departement inner join liste_modules lm on f.id_filiere = lm.id_filiere where (lm.prof_cours=:id_prof or lm.prof_TD=:id_prof or lm.prof_TP=:id_prof) and d.id_dprtm=:id_dprtm;");
		$sth->bindValue(':id_prof', $_SESSION['professeur']['id_prof']);
        $sth->bindValue(':id_dprtm', $_POST['departement']);
        $sth->execute(); 
		$result = $sth->fetch();
		$count=$sth->rowCount();
	if ($_SESSION['professeur']['anciennete']>6 && $_SESSION['professeur']['grade']='A' && !empty($result)) {
...
<?php } ?>
  ...
  <div class="form-row">
	...
	<label for="CoordFiliere">Coordonnateur de filiere</label>
      <input type="text" required value="<?=$_SESSION['professeur']['nomPrenom_prof']?>" class="form-control" id="CoordFiliere">
