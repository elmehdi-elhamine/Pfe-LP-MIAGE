...
<body>
							...
							<ul class="dropdown-menu dropdown-user animated fadeIn">
								<div class="dropdown-user-scroll scrollbar-outer">
									...
									<li>
										...
										<div class="user-box"><a style="font-size: 13px;" class="btn btn-xs btn-secondary btn-sm" href="profile_admin.phpp">
										     Mon Profile</a>
										</div>
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<div class="table-responsive">
										<table id="add-row" class=" table  table-hover border " cellspacing="0" width="100%" >
											...
											<thead style="background-color: #D7D9E7;">
												<tr >
													...
													<th >candidats avoir maximum de vote</th>
													<th >nbr de vote</th>
												<th>valider les reultas</th>
  ...
  $sql = "SELECT max(y.voter) as gan, y.candidat as candi , y.id_filiere as id_filiere,y.id_dprtm as id_dprtm,y.id_prof as id_prof FROM 
      (
      SELECT COUNT(c.id_candidat) as voter ,p.nom_poste as poste,d.nom_dprtm,f.intitule_filiere as filiere ,f.id_filiere as id_filiere, d.id_dprtm as id_dprtm ,pr.id_prof as id_prof ,pr.nomPrenom_Prof as candidat FROM vote v 
...
LEFT JOIN filiere f on f.id_filiere=p.id_filiere
left join departement d on d.id_dprtm=p.id_departement
 where p.id_poste='$row->id_poste'
 GROUP BY c.id_candidat) as y";

                                           $statement = $db->prepare($sql);
                                            $statement->execute();
                                            $resulats = $statement->fetchAll(PDO::FETCH_OBJ);
                                            foreach ($resulats as $max) {?>
												
											      <td> 
										      <?php if (empty($max->candi)) {?>
											<h5 class="card-footer text-muted" style="font-weight: bold;">aucun</h5>
									          <?php } else {?>

									          	<form method="post" action="valider_vote.php">
													...
													<input type="hidden" name="coorFiliere" value="<?php echo $max->id_prof; ?>"> 
													<input type="hidden" name="chefDepartement" value="<?php echo $max->id_prof; ?>">

                                                    <input type="hidden" name="id_dprtm" value="<?php echo $max->id_dprtm; ?>">
                                                    <input type="hidden" name="id_filiere" value="<?php echo $max->id_filiere; ?>">
                                                    <input type="hidden" name="nom_poste" value="<?php echo $row->nom_poste; ?>">



													<p style="color: ;font-weight: bold;"><?php echo $max->candi; }?>
													...
													<td ><?php echo $max->gan;; ?></td>
													<td style="font-weight: bold;" >
                                         
                                            <?php if ($date>$date_election) {?>
											<button onclick="return confirm('êtes-vous sûr?');" type="submit" class="btn btn-info" name="valider" >
												<span class="btn-label">
													<i class="fas fa-check"></i>
												</span> valider
											</button> <?php ;} else {   ?>
											<a href="" role="button" onclick="return confirm('le vote pas en cours terminer . vous ne pouvez pas affecter les resultas maintenant ');" class="btn btn-link"  >
												<span class="btn-label">
