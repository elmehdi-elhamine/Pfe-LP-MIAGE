 ...
 <div class="card" style="background-color:blue; padding: 0; margin:0;">
       ...
       <div id="collapseThree<?php echo $rowc->nom_cycle?><?php echo $row->nom_dprtm ?>" class="collapse" aria-labelledby="headingThree" data-parent="#accordion">
             ...
             <!---------------------- filieres ----------------------->
                ...
                <?php 
                  $sqlf = "SELECT * FROM  filiere f INNER join departement d on f.id_departement = d.id_dprtm INNER join cycle
                           c on f.id_cycle = c.id_cycle LEFT join professeur p on f.coorFiliere = p.id_prof where f.id_departement = '$dprtm' and f.id_cycle = '$cycle' ";
     ...
     <?php if($rowf->nom_cycle == "Licence"){?>
        ...
        <!-- Modal body -->
                   <div class="modal-body">
                            ...
                            <?php
                            $sth = $db->prepare("SELECT m.intitule_module, m.id_module as idmodule from module m where m.id_departement = '$dprtm' ");
                            $sth->execute(); 
                            $rowm=$sth->fetchAll(PDO::FETCH_OBJ); ?>
                            <select required class="form-control" name="module" id="module">
							<option selected>Choose...</option>
							              <?php foreach ($rowm as $rowm) {?>
                            <option value="<?php echo $rowm->idmodule ; ?>"><?php echo $rowm->intitule_module ;?></option><?php } ?>
