...
$COUNT=0;
       ...
       $requete = "SELECT count(*) as som FROM `liste_modules` WHERE (prof_cours is null OR prof_TD is null OR prof_TP is null)  and id_module = '$id_module' and id_filiere = '$id_filiere'";
   ...
   if (!empty($id_profC)) {
         ...
         $sql = "UPDATE liste_modules SET prof_cours = '$id_profC' WHERE id_module = '$id_module' and id_filiere = '$id_filiere'  ";             
             ...
             $statement = $db->prepare($sql);
                        ...
                        $statement->execute();
                                  $_SESSION['success'] = "<strong style=color:green>votre modification a bien effectuer</strong>";
   ...
   if (!empty($id_profTD)) {
    ...
    if(empty($id_profC) && empty($id_profTD) && empty($id_profTP)){
        $_SESSION['error'] = "<strong style=color:red>Vous n'avez affecter aucun professur !</strong>" ;
           ...
           header('location:module_prof.php');
           
       }
       else {header('Location:erreur_affecter_prof.php');}}
       	//echo 'ce champs deja inserer ';}
       ?>
